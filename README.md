...
**`DualVideo-v2.1.apk`**（约 4.7 MB，已签名，Android 7.0+ / minSdk 24）

1. 传到手机（微信/QQ/USB/蓝牙均可），点击安装，允许"未知来源应用"。
2. 打开 → 「选择本地视频」→ 选一个视频 → 左右双屏同步播放。
3. 已装过旧版可直接覆盖安装（同一签名，versionCode 12）。
...
| 畸变 | 控制条「画面」→「畸变」按钮循环切换 关 / 桶形 / 枕形 + 角度滑杆（0°~180°）。畸变作用在**半屏画幅**上，左右各一个固定中心，不随画面缩放/位移改变 |
| 镜心 | 控制条「画面」→「镜心」滑杆，调节两侧畸变中心（镜片光心）的间距：靠拢 = 缩小等效瞳距，分开 = 加大 |
| 推荐 | 控制条「画面」→ 重置 / 填满 / 靠拢 / 桶形 / 枕形，一键套用常用组合 |
...
- **v2.1**：去掉「远近 / 镜心」两个手动水平控件，改为**标准画幅模式**（原始 / 全景 2:1 / 1:1 / 16:9 / 9:16），导入时按视频横竖屏自动匹配并居中裁剪；画面中心与畸变中心在渲染层面恒等；两幅画面固定在中轴相邻摆放，中心间距 = 一个画面宽度，从而避免手机过宽导致两眼无法汇聚。
- **v2.0**：畸变中心（镜片光心）可整体靠拢/分开。新增「镜心」滑杆：左镜心 = `−0.5 + lensShift`、右镜心 = `+0.5 − lensShift`（仍严格镜像）。用于匹配 VR 盒两片镜片的实际间距，避免两个畸变中心离得太远导致人眼无法汇聚。预设也一并带上镜心值。
- **v1.9**：畸变从"画面坐标系"改为"半屏画幅坐标系"（左右各一固定中心，不随缩放/位移变化），改为桶形/枕形可切换并支持正负强度；新增 5 个快捷推荐预设（重置 / 填满 / 靠拢 / 桶形 / 枕形），「填满」会按视频宽高比与半屏宽高比自动算出铺满所需的缩放倍数。
